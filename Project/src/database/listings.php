<?php
    ...
    function getOwnerListingsFilter($owner, $types=array(), $priceLow=0, $priceHigh='PHP_INT_MAX', $city='%', $checkdates = false, $checkin='now', $checkout='01-01-2300') {
        $db = Database::instance()->db();
        ...
        if($checkdates){
            foreach ($types as $type) {
                $stmt = $db->prepare('SELECT * 
                                      From Property AS P
                                      WHERE user_id = ? AND price_day >= ? AND price_day <= ? 
                                      AND city LIKE ? AND property_type = ? AND id NOT IN (
                                          SELECT DISTINCT Property.id
                                          FROM Property, Rented 
                                          WHERE Property.id = property_id AND (julianday(?) BETWEEN julianday(initial_date) AND julianday(final_date)
                                          OR julianday(?) BETWEEN julianday(initial_date) AND julianday(final_date)
                ...
                $stmt->execute(array($owner, $priceLow, $priceHigh, $city, $type, $checkin, $checkout, $checkin));
                $return = array_merge($return, $stmt->fetchAll());
            }
        }
        else {
            foreach ($types as $type) {
                $stmt = $db->prepare('SELECT * 
                                      FROM Property WHERE user_id = ? AND
                                      price_day >= ? AND price_day <= ? 
                                      AND city LIKE ? AND property_type = ?');
                $stmt->execute(array($owner, $priceLow, $priceHigh, $city, $type));
                $return = array_merge($return, $stmt->fetchAll());
            }
        }
